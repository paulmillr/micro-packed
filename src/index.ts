...
const lengthCoder = (len: Length) => {
  ...
  const normalizeLength = (byteLen: unknown, err: (msg: string) => Error): number => {
    if (typeof byteLen === 'bigint') byteLen = Number(byteLen);
    else if (typeof byteLen !== 'number') throw err(`Wrong length: ${byteLen}`);
    ...
    if (!isNum(byteLen) || byteLen < 0) throw err(`Wrong length: ${byteLen}`);
    return byteLen;
  };
  return {
    ...
    decodeStream(r: TArg<Reader>) {
      ...
      return normalizeLength(byteLen, (msg) => r.err(msg));
    },
  };
};
...
function validateReaderOpts(opts: ReaderOpts): void {
